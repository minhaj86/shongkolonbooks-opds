...
class Format {
     ...
     * ATOM output format
     */
    const ATOM_FORMAT = 'atom+xml';

    /**
     * Default format of this class
     */
    const DEFAULT_FORMAT = self::ATOM_FORMAT; // Couldn't be DEFAULT, as this is a keyword
     ...
     * @return mixed
     */
    public function to_xml($data = NULL, $structure = NULL, $basenode = 'entry')
    {
        if ($data === NULL && func_num_args() === 0)
        {
            $data = $this->_data;
    ...
    $options = array(
      ...
      "encoding"        => "UTF-8",
      "rootName"        => $basenode,
      "rootAttributes"  => array("version" => "0.91" , 'xmlns'=>"http://www.w3.org/2005/Atom", 'xmlns:dc' => "http://purl.org/dc/terms/", 'xmlns:opds' => "http://opds-spec.org/2010/catalog"),
      "defaultTagName"  => "item",
...
return $serializer->getSerializedData();
     ...
     * @return mixed
     */
    public function to_atom($data = NULL, $structure = NULL, $basenode = 'feed')
    {
        if ($data === NULL && func_num_args() === 0)
        {
            $data = $this->_data;
